...
async def test_when_valid_activity_then_returns_activity_code(enumerate_model):
    ...
    assert activity_code == 123


@pytest.mark.asyncio
async def test_when_invalid_activity_then_raises(enumerate_model_empty):
    with pytest.raises(ValueError):
        await enumerate_model_empty.get_activity()


@pytest.mark.asyncio
async def test_when_valid_birth_place_then_return_combination_birth_place(enumerate_model):
    ...
    assert birth_place_combination.get("city") == 102030


@pytest.mark.asyncio
async def test_when_invalid_birth_place_combination_then_raises(enumerate_model_empty):
    with pytest.raises(ValueError):
        await enumerate_model_empty.get_combination_birth_place()
...
async def test_when_valid_address_then_return_address_combination(enumerate_model):
    ...
    assert address_combination.get("state") == "BA"
    assert address_combination.get("city") == 203040


@pytest.mark.asyncio
async def test_when_invalid_address_combination_then_raises(enumerate_model_empty):
    with pytest.raises(ValueError):
        await enumerate_model_empty.get_combination_address()
...
async def test_when_have_foreign_account_tax_then_return_countries(enumerate_model):
    ...
    assert countries[0] == "ESP"


@pytest.mark.asyncio
async def test_when_no_foreign_account_tax_then_return_empty_list(enumerate_model_empty):
    countries = await enumerate_model_empty.get_country_foreign_account_tax()
...
@pytest.mark.asyncio
@patch('func.src.domain.user_enumerate.model.foreign_account_tax.get', return_value={})
async def test_when_invalid_foreign_tax_then_raises(mock_get_account_tax, enumerate_model):
    with pytest.raises(ValueError):
